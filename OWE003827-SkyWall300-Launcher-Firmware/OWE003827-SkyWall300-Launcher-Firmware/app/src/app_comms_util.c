...
uint16_t calc_msg_crc16(const uint8_t msg_buffer[], const uint8_t msg_len)
{
	...
	// Each example has some subtle differences in how it works to do with when the crc16 variable is left/right shifted
	uint16_t crc16 = 1000;
	int index;

	for (int i = 0; i < msg_len; i++)
	{
		index = (uint8_t)((crc16 >> 8) ^ msg_buffer[i]);     // Some examples show crc16 being right shifted
...
{
	app_add_uint16_to_buffer_LE(msg_buffer, (uint8_t*)msg_len, calc_msg_crc16(msg_buffer, *msg_len));
}